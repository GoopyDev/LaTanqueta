...
public class PowerUpAuto : MonoBehaviour
{
    ...
    private Vector3 ElegirPosicion()
    {
        ...
        {
            int a = random.Next(0, anchoMapa * 10);      //Generamos nro random entre 0 y anchoMapa para la coordenada X
            int b = random.Next(0, profundidadMapa * 10);//Generamos nro random entre 0 y profundidadMapa para la coordenada Z
                                                         //Nota: la coordenada Y siempre será 0.5
            ...
            if (!Physics.CheckSphere(new Vector3(a, 1.1f, b), 0.8f)) //Physics.CheckSphere evalua si hay un Collider en la posicion, en un radio de 0.8f
            {
                posicionElegida = new Vector3(a, 0.5f, b); //Aquí asignamos los valores, ya que tendríamos una ubicación libre
    ...
    void Start()
    {
        ...
        //MapGenerator datosMapa = mapa.GetComponent<MapGenerator>();
        //anchoMapa = datosMapa.ancho;
        //profundidadMapa = datosMapa.profundidad;
    }
